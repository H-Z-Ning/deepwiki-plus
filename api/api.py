...
@app.post("/structural-analysis")
async def structural_analysis(request: Request):
    logger.info("structural_analysis")
    logger.info("request")
    try:
        ...
        if repo_type == 'local':
            ...
            if not local_path:
                return {"status": "error", "message": "The local project path cannot be empty."}
            project_dir = local_path
        else:
            # GitHub/GitLab等远程仓库，使用克隆后的本地目录
            repo_url = data.get('repo_url')
            owner = data.get('owner')
            repo = data.get('repo')

            # 构造本地存储路径（与data_pipeline.py中的逻辑一致）
            root_path = get_adalflow_default_root_path()
            project_dir = os.path.join(root_path, "repos", owner+"_"+repo)

            if not os.path.exists(project_dir):
                return {"status": "error", "message": f"The project directory does not exist.: {project_dir}"}

                # 调用 project_parser.py 中的 parser 方法
        from api.tools.project_parser import parse_project
        logger.info("-------------------project_dir--------------------------- %s", project_dir)
        analysis_result = parse_project(project_dir)  # 传递本地目录路径

        return {"status": "success", "analysis": analysis_result}
    except Exception as e:
        return {"status": "error", "message": str(e)}

@app.post("/get-structural-analysis")
async def get_structural_analysis_api(request: Request):
    logger.info("get-structural-analysis")
    try:
        data = await request.json()
        repo_type = data.get('type', 'github')
        ...
        if repo_type == 'local':
            ...
            if not os.path.exists(project_dir):
                ...
                # 调用 project_parser.py 中的 parser 方法
        from api.tools.project_parser import get_structural_analysis
        logger.info("-------------------project_dir--------------------------- %s", project_dir)
        analysis_result = get_structural_analysis(project_dir)  # 传递本地目录路径
